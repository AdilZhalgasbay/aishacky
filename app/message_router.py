...
def auto_route_message(text: str, sender: str) -> tuple[str, dict[str, Any] | None]:
    ...
    text_l = text.lower()
    attendance_kw = [
        "детей", "присутствуют", "болеют", "присутствует", "отсутствуют", "человек", "питание", "порции", "порций", "на уроке", "ученика", "классе"
    ]
    absence_kw = [
        "заболел", "заболела", "болею", "плохо", "больнице", "справка",
        "курсах", "курсы", "олимпиада", "выезд", "конференция",
        "отгул", "отпуск", "за свой счет",
    ...
    incident_kw = [
        "сломался", "поломка", "не работает", "проблема", "нужен", "нужна", "замените",
        "разбито", "течет", "горит", "дымит", "застрял", "пропал", "украли",
        "мышка", "замок", "дверь", "кран", "вода", "лампочка", "свет", "монитор", "клавиатура"
    ]
    resolution_kw = [
        "сделал", "сделала", "починил", "починила", "готово", "исправлено", "решено", "заменил", "заменила"
    ]
